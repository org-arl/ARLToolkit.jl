...
function gpx_segment2pts(segment; filename=filename, tz=localzone())
    p = segment.points
...
function read(filename; tz=localzone())
  ...
  length(gpx.tracks[1].segments) > 1 && return gpx_segment2pts.(gpx.tracks[1].segments; filename=filename, tz=tz)
